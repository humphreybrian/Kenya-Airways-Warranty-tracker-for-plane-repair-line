...
if(!empty($_POST["add_record"])) {
    require_once("db.php");
    $sql = "INSERT INTO ITEMS ( UNIT, PARTNUMBER, SERIALNUMBER, DATECD, DATEMVD, ACTYPE, ACREG, POS, HOURSRUN, QTY, DEFECT, TECH, CATEGORY, BLR, QUARANTINE, MODSTATUS) VALUES ( :unit, :partnumber, :serialnumber, :datecd, :datemvd, :actype, :acreg, :pos, :hoursrun, :qty, :defect, :tech, :category, :blr, :quarantine, :modstatus, )";
    $pdo_DATECOMPLETEDment = $DB_con->prepare( $sql );
        
    $result = $pdo_DATECOMPLETEDment->execute( array( ':unit'=>$_POST['unit'], ':partnumber'=>$_POST['partnumber'], ':serialnumber'=>$_POST['serialnumber'], ':datecd'=>$_POST['datecd'], ':datemvd'=>$_POST['datemvd'], ':actype'=>$_POST['actype'], ':acreg'=>$_POST['acreg'], ':pos'=>$_POST['pos'], ':hoursrun'=>$_POST['hoursrun'], ':qty'=>$_POST['qty'], ':defect'=>$_POST['defect'], ':tech'=>$_POST['tech'], ':category'=>$_POST['category'], ':blr'=>$_POST['blr'],  ':quarantine'=>$_POST['quarantine'], ':modstatus'=>$_POST['modstatus']) );
    if (!empty($result) ){
