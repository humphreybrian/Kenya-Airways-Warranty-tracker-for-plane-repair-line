...
include 'db.php';
    ...
    function getData($sql){
        global $DB_con ;//we use connection already opened
    ...
    if(!empty($_POST['search']['value'])){
        ...
        $sql = sprintf("SELECT * FROM %s %s ORDER BY %s %s  ", MyTable , $where ,$orderBy, $orderType  );
        $data = getData($sql);
    }
    /* END SEARCH */
    else {
        $sql = sprintf("SELECT * FROM %s ORDER BY %s %s ", MyTable ,$orderBy,$orderType );
        $data = getData($sql);
